...
export default function AdminDashboard() {
  ...
  const fetchBasvurular = async () => {
    try {
      ...
      if (selectedBasvuru) {
        const updatedBasvuru = data.find((b: AdminBasvuru) => b.id === selectedBasvuru.id)
        if (updatedBasvuru) {
  ...
  }, [searchTerm, filterTarihBaslangic, filterTarihBitis, filterSinif, filterOkul, filterSinav])

  const filteredIds = useMemo(() => filteredBasvurular.map((b) => b.id), [filteredBasvurular])
  ...
  const handleArchiveSelected = async () => {
    ...
    if (
      !confirm(
        `${n} başvuru arşivlenecek. Ana listede görünmeyecek; veri silinmeyecek. Onaylıyor musunuz?`
      )
    ) {
      return
    }
    setIsArchiving(true)
    try {
      const res = await fetch('/api/admin/basvurular/arsivle', {
        method: 'POST',
      ...
      const arsivlenen = data.arsivlenen ?? 0
      const istenen = data.istenen ?? n
      if (arsivlenen < istenen) {
        alert(
          `${arsivlenen} kayıt arşivlendi. ${istenen - arsivlenen} kayıt şube kapsamında bulunamadı veya zaten arşivdeydi.`
        )
      } else {
        alert(`${arsivlenen} kayıt arşivlendi.`)
      }
      setSelectedIds(new Set())
      fetchBasvurular()
    } catch (e) {
        ...
        <div className="bg-white rounded-lg shadow overflow-hidden">
          <div className="flex flex-wrap items-center justify-between gap-3 px-4 py-3 border-b border-gray-200 bg-gray-50">
            <div className="text-sm text-gray-600">
              {selectedIds.size > 0 ? (
                <span className="font-medium text-gray-900">{selectedIds.size} seçili</span>
              ) : (
                <span>Filtrelenen listeden seçim yapın veya tümünü seçin</span>
              )}
            </div>
            <div className="flex flex-wrap gap-2">
              <button
                ...
                disabled={filteredBasvurular.length === 0}
                className="text-sm px-3 py-2 border border-gray-300 rounded-lg hover:bg-white disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {allFilteredSelected ? 'Listeyi temizle' : 'Filtrelenen tümünü seç'}
              </button>
              <button
                type="button"
                onClick={handleArchiveSelected}
                disabled={selectedIds.size === 0 || isArchiving}
                className="text-sm px-4 py-2 bg-amber-700 text-white rounded-lg hover:bg-amber-800 disabled:opacity-50 disabled:cursor-not-allowed"
              >
                {isArchiving ? 'Arşivleniyor…' : 'Seçilenleri arşivle'}
              </button>
            </div>
          </div>
            ...
            <table className="min-w-full divide-y divide-gray-200">
              ...
              <tbody className="bg-white divide-y divide-gray-200">
                {filteredBasvurular.length === 0 ? (
                  <tr>
                    <td colSpan={isRize ? 7 : 6} className="px-6 py-12 text-center text-gray-500">
                  ...
                  filteredBasvurular.map((basvuru, index) => (
                    ...
                    >
                      <td className="px-3 py-4 whitespace-nowrap" onClick={(e) => e.stopPropagation()}>
                        <input
                          type="checkbox"
                          checked={selectedIds.has(basvuru.id)}
                          onChange={() => toggleSelectRow(basvuru.id)}
                          className="rounded border-gray-300 text-indigo-600 focus:ring-indigo-500"
                        />
                      </td>
